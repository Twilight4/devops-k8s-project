* TODO Project overview
- 3 microservices (small, independent):
  + *api* — front-end API (Node.js / Express)
  + *users* — user service (CRUD, REST)
  + *orders* — order service (CRUD, REST)
- *Database*: PostgreSQL (one central DB or one per service — start with a single DB via PVC)
- *Ingress*: HTTP routing with host/path rules
- *Config*: ConfigMaps & Secrets
- *Persistence*: PersistentVolumeClaim for Postgres
- *Deployment*: Deployments + Services + Horizontal Pod Autoscaler (HPA)
- *Packaging*: Helm chart for the app
- *CI/CD*: GitHub Actions to build/push images and deploy to cluster (minikube for demo; optional EKS/GKE/AKS)

* Quick prerequisite checklist
Install (locally):
- git
- Docker (or Docker Desktop)
- kubectl
- minikube or kind (I recommend kind for reproducible CI-friendly clusters)
- helm (v3)
- (optional) kubectl port-forward or kubectl proxy for demos
  
* Repo layout
#+begin_src shell
devops-k8s-project/
├─ README.org
├─ .github/
│  └─ workflows/
│     └─ ci-cd.yml
├─ charts/
│  └─ microservices/
│     ├──  templates
│     │   ├──  api-gateway.yaml
│     │   ├──  ingress.yaml
│     │   ├──  postgres-pvc.yaml
│     │   ├──  postgres.yaml
│     │   ├──  orders.yaml
│     │   ├──  users.yaml
│     ├──  charts
│     ├──  values.yaml
│     └──  Chart.yaml
├─ deploy/
│  ├─ users-deployment.yaml
│  ├─ orders-deployment.yaml
│  ├─ api-gateway-deployment.yaml
│  ├─ postgres-deployment.yaml
│  ├─ postgres-pvc.yaml
│  └─ ingress.yaml
├─ services/
│  ├─ users-service/
│  │  ├─ Dockerfile
│  │  ├─ src/
│  │  ├─ package-lock.json (generated)
│  │  └─ package.json
│  ├─ orders-service/
│  │  ├─ Dockerfile
│  │  ├─ src/
│  │  ├─ package-lock.json (generated)
│  │  └─ package.json
│  └─ api-gateway/
│     ├─ Dockerfile
│     ├─ src/
│     ├─ package-lock.json (generated)
│     └─ package.json
└─ docs/
   └─ architecture.png
#+end_src

* Build and test locally minimal microservices
It is using Node.js Express service to replicate for =api=, =users=, =orders=.

#+begin_src shell
# Log in to dockerhub
docker login

# Build and run the users image (I'm using my own hub name)
cd services/users-service/
npm install                         # Run this command to generate package-lock.json file needed for docker build command
docker build -t twilight4/users:0.1.0 .
docker run -p 3001:3001 twilight4/users:0.1.0

# test the users image
docker ps
curl http://localhost:3001/users

# Build and run the orders image
cd services/orders-service/
npm install                        # Run this command to generate package-lock.json file needed for docker build command
docker build -t twilight4/orders:0.1.0 .
docker run -p 3002:3002 twilight4/orders:0.1.0

# test the orders image
docker ps
curl http://localhost:3002/orders

# Build and run the api-gateway image
cd services/api-gateway/
npm install                        # Run this command to generate package-lock.json file needed for docker build command
docker build -t twilight4/api:0.1.0 .
# To check docker's IP: ip route | grep docker (default on linux is 172.17.0.1)
docker run -it -p 3000:3000 \
  -e USERS_URL=http://172.17.0.1:3001 \
  -e ORDERS_URL=http://172.17.0.1:3002 \
  twilight4/api:0.1.0

# test the api-gateway image
docker ps
curl http://localhost:3000/users
curl http://localhost:3000/orders
curl http://localhost:3000/health
#+end_src

* Create Cluster
** Prerequisite
Create a secret for DB password:
#+begin_src shell
# Create secret
kubectl create secret generic pg-secret --from-literal=password='changeme' -n <demo/default>
#+end_src

For ingress controller, you must modify =/etc/hosts= to point to =micro.local= to the cluster LB:
#+begin_src shell
# Make sure /etc/hosts has
# 192.168.49.2 micro.local
#+end_src

** Using kind
#+begin_src shell
kind create cluster --name micro-cluster

# If using nginx-ingress for local
kubectl apply -f https://raw.githubusercontent.com/kubernetes/ingress-nginx/controller-v1.8.1/deploy/static/provider/kind/deploy.yaml

# Build images locally
docker build -t twilight4/users:0.1.0 ./services/users-service
docker build -t twilight4/api:0.1.0 ./services/api-gateway
docker build -t twilight4/orders:0.1.0 ./services/orders-service
# OR pull from dockerhub

# Verify images were built and load into kind
docker images | grep twilight4
kind load docker-image twilight4/users:0.1.0 --name micro-cluster
#+end_src

** Using minikube
#+begin_src shell
minikube start
minikube addons enable ingress

# Build image locally
eval $(minikube docker-env)
docker build -t twilight4/users:0.1.0 ./services/users-service
docker build -t twilight4/api:0.1.0 ./services/api-gateway
docker build -t twilight4/orders:0.1.0 ./services/orders-service
# OR pull from dockerhub

# Verify images were built
docker images | grep twilight4
#+end_src

* Deploy Cluster
** Using Helm packaging
#+begin_src shell
# Install
helm install my-micro charts/microservices -n demo --create-namespace

# Upgrade
helm upgrade my-micro charts/microservices -n demo

# Uninstall
helm uninstall my-micro -n demo
#+end_src

** Using kubectl
#+begin_src shell
# Apply manifests
kubectl apply -f deploy/
kubectl get pods -n default
kubectl get ingress
#+end_src

** Test endpoints
#+begin_src shell
# Test URLs
minikube service list

# Users and Orders service (through API Gateway)
curl http://micro.local/api/users
curl http://micro.local/api/orders
# API Gateway health
curl http://micro.local/api/health
#+end_src

* CI/CD with GitHub Actions (basic flow)
The =.github/workflows/ci-cd.yml=:
1. Builds Docker images
2. Pushes images to Docker Hub / GitHub Container Registry
3. Deploys to cluster (for demo, use =kubectl apply= to minikube/kind in a separate runner or use GitHub Actions runner with kubeconfig stored assecret; for production, use something like ArgoCD)

Deployment step (simple): run =kubectl set image= or =helm upgrade= using a =kubeconfig= stored as secret. For learning demo, you can skip automated deploy and instead use a manual =workflow_dispatch= deploy that you run from Actions.
